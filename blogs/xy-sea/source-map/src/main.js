...
function resourceTransform(target) {
  return {
    ...
    source_url: `资源加载失败： ${
      target.src.slice(0, 100) || target.href.slice(0, 100)
    } `,
...
function loadSourceMap(fileName) {
  ...
  return new Promise(resolve => {
    fetch(`${window.location.origin}/getmap?fileName=${file}`).then(
      response => {
...
const findCodeBySourceMap = async ({ fileName, line, column }) => {
  ...
  target.innerHTML = `<div class="errdetail"><div class="errheader">${
    result.source
