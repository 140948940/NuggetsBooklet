...
app.get('/readline', function (req, res) {
  ...
  objReadline.on('close', function () {
    res.send(
      arr.toString().replace(/,,/g, ',').replace(/{,/g, '{').replace(/},/g, '}')
    )
  })
})
...
app.get('/writeFile', function (req, res, next) {
  fs.writeFile('fs/index.js', '写入内容', err => {
    // 如果要写入的文件(index.js)不存在, 系统会自动创建一个index.js文件，并将内容写入文件，注意: 该函数不能连续写入，只能写入一次，不能连续写入
    if (err) {
      console.log(err)
      res.send(err)
    } else {
...
app.get('/appendFile', function (req, res, next) {
  fs.appendFile('fs/fs.js', `\n追加内容写入的内容`, err => {
    // 如果要写入的文件(index.js)不存在, 系统会自动创建一个index.js文件，并将内容连续写入文件
    if (err) {
      console.log(err)
      res.send(err)
    } else {
